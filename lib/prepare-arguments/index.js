...
module.exports = function (initialParams, taskList, endFunction) {
	...
	switch (types) {
		...
		case 'array,function,undefined':
			retValue = createSafeParameters({}, initialParams, endFunction);
			break;
		...
		case 'object,array,function':
			...
			break;
		default:
			throw new Error('Invalid signature found. The first argument must be an object (for initial arguments, when this case, the next argument must be an array (for tasklist)) or an array (for tasklist). The last argument may a function.  But got "' + types + '"');
	}
